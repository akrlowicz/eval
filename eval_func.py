...
def plot_test_comparison(clfs, X_test, y_test):
  
  
  acc = [accuracy_score(y_test,clf.predict(X_test)) for clf in clfs]
  f1 = [f1_score(y_test,clf.predict(X_test), average='weighted') for clf in clfs]
  precision = [precision_score(y_test,clf.predict(X_test), average='weighted') for clf in clfs]
  recall = [recall_score(y_test,clf.predict(X_test), average='weighted') for clf in clfs]


  width = 0.1
  x= np.arange(len(acc))
  fig, ax = plt.subplots(figsize=(12,7))
  ax.bar(x - 2*width, acc, width, label='accuracy', alpha=0.8)
